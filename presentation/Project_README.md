<div align="center">
  ...
  > Tối ưu hóa phân tải mạng SDN bằng mô hình **TFT-CQL Actor-Critic** (Temporal Fusion Transformer + Conservative Q-Learning) trong môi trường Mininet mô phỏng hệ thống đăng ký tín chỉ đại học (LMS) dưới tải cao.
</div>
...
Clients (h9–h16)
     ...
     ▼
[Ryu Controller — TFT-CQL]  ←→  flow_stats.csv
     │  OpenFlow NAT
...
**Thuật toán:** RR | WRR (1:2:3) | **TFT-CQL** (Inference < 100ms)
...
3. Controller (TFT-CQL) chọn backend tốt nhất (h5/h7/h8)
...
## Phương hướng AI — Huấn luyện TFT-CQL Actor-Critic

### Tại sao TFT + CQL Actor-Critic?
...
| **CQL Actor-Critic (Conservative Q-Learning)** | Dựa trên dự đoán của TFT để **ra quyết định** chuyển luồng sang server nào. Học offline qua thưởng/phạt từ lịch sử với cơ chế conservative để tránh overestimation. |
| **Kết hợp** | TFT dự báo → Actor-Critic hành động → Hệ thống tự tiến hóa |
...
Bước 4: INFERENCE (controller_stats.py)
  ...
  ├── TFT-CQL forward pass → Q-values cho 3 server
  └── Chọn action bằng sampled/deterministic policy → bẻ luồng OpenFlow NAT
```
...
### Kịch bản 1 — Golden Hour: Cơn lốc Đăng ký
`golden_hour.yml`
...
**Ưu điểm của AI:** TFT nhận diện spike pattern trước 5 timestep, CQL bẻ luồng sang server còn rảnh.

---

### Kịch bản 2 — Video Conference: Họp trực tuyến
`video_conference.yml`

```
Tải trọng:  ___/‾‾‾‾\___/‾‾‾‾\ Ổn định, độ trễ yêu cầu cao
Pattern:    Băng thông ổn định, yêu cầu latency thấp
Thời gian:  ~15 phút
```
**Mục đích:** Kiểm tra khả năng **duy trì latency thấp** của AI. Tải ổn định nhưng yêu cầu chất lượng dịch vụ cao.
**Yếu điểm của RR/WRR:** Không ưu tiên server có latency tốt hơn.
**Ưu điểm của AI:** TFT nhìn ra pattern ổn định → cân tải tối ưu cho latency.

---

### Kịch bản 3 — Hardware Degradation: Suy giảm phần cứng
`hardware_degradation.yml`

```
Tải trọng:  ████░░░░███░░░░███  Một server dần mất khả năng
Pattern:    Server h5 dần bị giới hạn xử lý
Thời gian:  ~10 phút
```
**Mục đích:** Kiểm tra **failover** — khả năng phát hiện và né 1 server đang suy giảm hiệu năng.
**Yếu điểm của RR/WRR:** RR vẫn tiếp tục gửi 1/3 request về h5 dù nó đang yếu đi.
**Ưu điểm của AI:** CQL học được rằng action=0 (h5) cho reward giảm dần → giảm xác suất chọn h5.

---

### Kịch bản 4 — Low-Rate DoS: Tấn công từ chối dịch vụ
`low_rate_dos.yml`

```
Tải trọng:  _____/‾‾‾‾‾‾‾‾‾‾‾‾‾ Tấn công ngầm, khó phát hiện
Pattern:    NORMAL → Nghẽn từ từ do tấn công ngầm
Thời gian:  ~15 phút
```
**Mục đích:** Kiểm tra **phát hiện tấn công** — AI phải nhận ra server đang bị nhắm mục tiêu.
**Yếu điểm của RR/WRR:** Không thể điều chỉnh weight theo thời gian thực khi bị tấn công.
**Ưu điểm của AI:** CQL nhận ra pattern bất thường → điều phối traffic tránh server bị tấn công.
...
| # | File | Mô tả |
|---|------|-------|
| 1 | `golden_hour.yml` | Cơn lốc truy cập đột ngột |
| 2 | `video_conference.yml` | Họp trực tuyến, yêu cầu latency thấp |
| 3 | `hardware_degradation.yml` | Suy giảm phần cứng server |
| 4 | `low_rate_dos.yml` | Tấn công từ chối dịch vụ tốc độ thấp |
...
│
├── ai_model/
│   ├── tft_ac_net.py         # Kiến trúc mô hình (TFT + Actor-Critic)
│   ├── cql_agent.py          # CQL Agent (Conservative Q-Learning)
│   ├── sdn_env_v2.py         # Môi trường Offline cho RL training
...
└── stats/
    ...
    └── results/
        ├── RR_golden_hour/       # Ket qua tu tung thuat toan x kich ban
        ├── WRR_golden_hour/
        ├── CQL_golden_hour/
...
| Biến | Mô tả | Mặc định |
|------|-------|---------|
| `SCENARIO` | File kịch bản Artillery | `golden_hour.yml` |
