...
def train_tft_dqn():
    ...
    print(f"\n🚀 BAT DAU TRAINING Mo Hinh AI (Actions: {env.num_actions})")
    print("=" * 60)
    ...
    for episode in range(NUM_EPISODES):
        ...
        # Print progress
        action_stats = " ".join([f"{node['name']}={action_counts[i]}" for i, node in enumerate(BACKENDS)])
        print(f"  Epoch {episode+1:2d}/{NUM_EPISODES} | "
              f"Eps: {agent.epsilon:.3f} | "
              f"Reward: {total_reward:6.1f} | "
              f"Q-Loss: {avg_dqn_loss:.6f} | "
              f"Actions: {action_stats}")
...
def _plot_training_dashboard(rewards, dqn_losses, tft_losses, epsilons,
    ...
    if len(tft_preds) > 0:
        ...
        # Feature names dynamically from config
        feature_names = ['Byte Rate', 'Packet Rate'] + [f'Load {n["name"]}' for n in BACKENDS]
        base_colors = [COLORS['primary'], COLORS['accent'], COLORS['secondary'], COLORS['warning'], COLORS['danger'], '#8B5CF6', '#EC4899']
        colors_feat = [base_colors[i % len(base_colors)] for i in range(len(feature_names))]
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    server_labels = [f'{n["name"]} (Srv {i+1})' for i, n in enumerate(BACKENDS)]
    base_colors = [COLORS['primary'], COLORS['secondary'], COLORS['warning'], COLORS['accent'], COLORS['danger']]
    server_colors = [base_colors[i % len(base_colors)] for i in range(len(BACKENDS))]
    ...
    # TFT Prediction (First 2 core features)
    if len(tft_preds) > 0:
        preds_arr = np.array(tft_preds)
        actuals_arr = np.array(tft_actuals)
