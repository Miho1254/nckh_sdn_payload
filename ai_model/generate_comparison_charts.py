...
Sử dụng:
    python3 ai_model/generate_comparison_charts.py --scenario golden_hour
"""
...
ALGO_STYLES = {
    ...
    'AI':      {'color': '#10B981', 'label': 'TFT-CQL (AI)',          'marker': 'D', 'linestyle': '-'},
...
SCENARIO_NAMES_VI = {
    'golden_hour':          'Giờ cao điểm',
    'video_conference':     'Hội nghị Video',
    'hardware_degradation': 'Suy giảm phần cứng',
    'low_rate_dos':         'Tấn công DoS tốc độ thấp',
}
...
def plot_packet_loss(algo_data, scenario_name):
    ...
    # Switch 8 port mapping (tự động theo config)
    PORT_MAP = {str(node['port']): node['name'] for node in BACKENDS}
    servers = [node['name'] for node in BACKENDS]
    
    # Tạo map màu động (bắt đầu từ red, gold, green cho 3 node đầu, sau đó random nếu nhiều hơn)
    base_colors = ['#ef4444', '#f59e0b', '#10B981', '#3b82f6', '#8b5cf6', '#ec4899']
    server_colors = {node['name']: base_colors[i % len(base_colors)] for i, node in enumerate(BACKENDS)}
...
def plot_server_heatmap(algo_data, scenario_name):
    ...
    Dung port_stats.csv — Switch 8 tx_bytes tren cac port backend."""
    PORT_MAP = {str(node['port']): node['name'] for node in BACKENDS}
    servers = [node['name'] for node in BACKENDS]
...
def plot_inference_overhead(algo_data, scenario_name):
    ...
    if df is not None and not df.empty and 'inference_ms' in df.columns:
        ...
        try:
            ...
            from tft_ac_net import TFT_ActorCritic_Model
            ...
            if pth_files:
                ...
                # Auto-detect input_size
                vsn_key = 'vsn.weight_grn.fc1.weight'
                input_size = state_dict[vsn_key].shape[1] if vsn_key in state_dict else 5
                ...
                # Try Actor Critic
                try:
                    model = TFT_ActorCritic_Model(input_size=input_size, seq_len=5, hidden_size=hidden_size, num_actions=3).to(device)
            ...
            model.eval()
            times = []
            dummy = torch.randn(1, 5, input_size).to(device)
            for _ in range(200):
                t = time.perf_counter()
                with torch.no_grad():
                    model(dummy)
...
def plot_heterogeneous_load(algo_data, scenario_name):
    ...
    servers = [f"{n['name']} ({'Weak' if n['weight'] < 5 else 'Medium' if n['weight'] < 10 else 'Strong'})" for n in BACKENDS]
    capacities = CAPACITIES * 10 # Quy đổi weight sang Mbps (1 weight = 10Mbps) như trong topo
    
    # Lay traffic thuc te cua RR va AI tu port_stats
    num_backends = len(BACKENDS)
    load_mb = {'RR': [0] * num_backends, 'AI': [0] * num_backends}
    ...
    for algo in ['RR', 'AI']:
        ...
        for node in BACKENDS:
            dpid = str(node['dpid'])
            port = str(node['port'])
            # Filter s8_stats cho dpid và port của backend này
            node_stats = latest[(latest['datapath_id'].astype(str) == dpid) & (latest['port_no'].astype(str) == port)]
            node_bytes = node_stats['tx_bytes'].sum()
            backend_loads.append(node_bytes / 1e6)
            
        load_mb[algo] = backend_loads
    ...
    rects2 = ax.bar(x + width/2, load_mb['AI'], width, label='TFT-CQL (AI)', color=ALGO_STYLES['AI']['color'], alpha=0.9)
...
def plot_latency_per_server(algo_data, scenario_name):
    ...
    # Mapping port -> friendly name + capacity
    PORT_TO_NAME = {node['port']: node['name'] for node in BACKENDS}
    NAME_TO_CAP = {node['name']: node['weight'] * 10 for node in BACKENDS}
    
    # Bảng màu cho server
    base_colors = ['#EF4444', '#F59E0B', '#10B981', '#3B82F6', '#8B5CF6', '#EC4899']
    colors = {node['name']: base_colors[i % len(base_colors)] for i, node in enumerate(BACKENDS)}
    ...
    for algo in ['RR', 'AI']:
        ...
        # Map in_port -> name
        nat['server'] = nat['in_port'].map(PORT_TO_NAME)
        ...
        for srv_node in BACKENDS:
            ...
            cap = NAME_TO_CAP[srv]
            ax.plot(agg['elapsed'], agg['smooth'], label=f"{srv} (Capacity: {cap}Mbps)", 
...
def plot_link_utilization(algo_data, scenario_name):
    ...
    # Mapping
    capacities_mbps = {node['name']: node['weight'] * 10 for node in BACKENDS}
    PORT_TO_NAME = {node['port']: node['name'] for node in BACKENDS}
...
def main():
    ...
    parser.add_argument('--scenario', type=str, required=True, help='Scenario name (e.g. golden_hour)')
    ...
    if args.presentation:
        ...
        all_scenarios = {}
        for sc in ['golden_hour', 'video_conference', 'hardware_degradation', 'low_rate_dos']:
