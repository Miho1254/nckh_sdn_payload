...
#   ./scripts/regen_charts.sh flash_crowd (chi 1 kich ban)
# ================================================================
...
if tft_predictions and tft_actuals:
    n = min(100, len(tft_predictions), len(tft_actuals))
    if n > 0:
        ax.plot(range(n), tft_actuals[:n], color=ACCENT[0], linewidth=1.5, label='Actual', linestyle='--')
        ax.plot(range(n), tft_predictions[:n], color=ACCENT[1], linewidth=1.5, label='TFT Predicted')
        ax.fill_between(range(n), tft_actuals[:n], tft_predictions[:n], alpha=0.1, color=ACCENT[3])
else:
    ax.text(0.5, 0.5, 'No TFT prediction data', ha='center', transform=ax.transAxes, fontsize=14)
ax.set_xlabel('Step'); ax.set_ylabel('Normalized Value')
ax.set_title('TFT Prediction vs Actual Traffic')
...
fig, ax = plt.subplots(figsize=(12, 5))
if action_counts:
    server_names = ['h5','h7','h8']
    colors_srv = [ACCENT[3], ACCENT[2], ACCENT[1]]
    n_epochs = len(action_counts)
    n_servers = len(action_counts[0]) if action_counts else 3
    x = list(range(n_epochs))
    bottoms = [0] * n_epochs
    for s in range(min(n_servers, 3)):
        vals = [action_counts[e][s] if s < len(action_counts[e]) else 0 for e in range(n_epochs)]
        ax.bar(x, vals, bottom=bottoms, color=colors_srv[s], label=server_names[s], alpha=0.85)
...
data_map = [
    ...
    (tft_predictions[:50] if tft_predictions else [], 'TFT Pred', ACCENT[4]),
    (tft_actuals[:50] if tft_actuals else [], 'Actual Traffic', ACCENT[0]),
]
for ax, (data, label, color) in zip(axes_list, data_map):
    if data:
