...
SCENES = ['golden_hour', 'video_conference', 'hardware_degradation', 'low_rate_dos']
...
def analyze_flow_stats(csv_path):
    ...
    backend_names = [node['name'] for node in BACKENDS]
    backend_dist = Counter({name: 0 for name in backend_names})
    ...
    if os.path.exists(port_csv_path):
        ...
        def get_delta(d, p, idx):
            vals = [v[idx] for v in sw_port_data.get(str(d), {}).get(str(p), [])]
        ...
        for node in BACKENDS:
            backend_dist[node['name']] = get_delta(node['dpid'], node['port'], 5) # tx_bytes
        
        # Total Real Traffic (What was sent by clients)
        # We now dynamically determine this based on configuration
        from config import TEST_CLIENTS_DETAILS
        for client in TEST_CLIENTS_DETAILS:
            actual_total_bytes += get_delta(client['dpid'], client['port'], 1) # rx_bytes
    ...
    BW_CAPACITIES = {node['name']: node['weight'] * 10 for node in BACKENDS}
    utilization_dist = {}
    ...
    # Tính throughput improvement so với WRR baseline
    # WRR distribution: [0.0625, 0.3125, 0.625] cho h5, h7, h8
    # AI distribution: backend_dist normalized
    wrr_distribution = {'h5': 0.0625, 'h7': 0.3125, 'h8': 0.625}
    wrr_capacity_weighted = sum(wrr_distribution[srv] * BACKENDS[i]['weight'] for i, srv in enumerate(['h5', 'h7', 'h8']))
    
    total_dist = sum(backend_dist.values()) if backend_dist else 1
    ai_distribution = {srv: cnt / total_dist for srv, cnt in backend_dist.items()} if backend_dist else {'h5': 0, 'h7': 0, 'h8': 0}
    ai_capacity_weighted = sum(ai_distribution.get(srv, 0) * BACKENDS[i]['weight'] for i, srv in enumerate(['h5', 'h7', 'h8']))
    
    # Throughput improvement: AI capacity-weighted - WRR capacity-weighted
    # Nếu > 0: AI tốt hơn WRR trong việc tận dụng capacity
    throughput_improvement = ai_capacity_weighted - wrr_capacity_weighted
...
def print_grand_summary(all_results):
    ...
    # Phần 3: Throughput Improvement (Càng cao càng tốt - so với WRR)
    print_log(f"{C_CYAN}[3] Throughput Improvement (So với WRR - Càng cao = Càng tốt){C_NC}")
    for scene in SCENES:
        row_res = all_results.get(scene, {})
        if not row_res: continue
        ...
        for a in algos:
            if a in row_res:
                ti = row_res[a].get('throughput_improvement', 0)
                is_best = ti == max(r.get('throughput_improvement', 0) for r in row_res.values())
                color = C_GREEN if is_best else C_NC
                indicator = "★" if is_best else " "
                row += f" | {color}{ti:>9.3f}{indicator}{C_NC}"
            else:
                row += f" | {'-':>10}"
        print_log(row)
    ...
    print_log(f"  - Thuật toán {C_GREEN}AI (TFT-CQL){C_NC} được kỳ vọng sẽ chiếm ưu thế (có nhiều ★ nhất)")
    print_log(f"    trong các kịch bản có tính biến động cao như {C_CYAN}golden_hour{C_NC} và {C_CYAN}low_rate_dos{C_NC}.")
    print_log(f"  - Throughput Improvement > 0: AI tốt hơn WRR trong việc tận dụng capacity")
    print_log(f"{C_BOLD}{C_BLUE}================================================================={C_NC}\n")
    ...
    for scene in scenes_to_run:
        ...
        for algo in ALGOS:
            ...
            if r:
                results[algo] = r
                import json
                out_json = os.path.join(RESULTS_DIR, f'{algo}_{scene}', 'evaluation_metrics.json')
                try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scenario', type=str, help='Specific scenario to report on (e.g. golden_hour)', default=None)
